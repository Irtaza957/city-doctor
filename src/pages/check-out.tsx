...
const CheckoutDetails = () => {
  ...
  const [createTabbyCheckout] = useCreateTabbyCheckoutMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        if (payMethod === "Tabby") {
          const tabbyUrlencodedToken = new URLSearchParams();
        ...
        if (
          ...
          urlencodedStatus.append(
            "reference",
            paymenttokenResp?.data?.data?.orderReference
          );
        ...
        if (showCard && !payMethod) {
          ...
          if (response?.session_id) {
            ...
            console.log(
              document.getElementById("3ds_iframe"),
              "3ds_iframe3ds_iframe"
            );
            const { status, error } = await window.NI.handlePaymentResponse(
            ...
            urlencodedStatus.append(
              "reference",
              paymentResponse?.data?.data?.orderReference
            );
            urlencodedStatus.append("booking_id", data.data.data.id);
    ...
    <>
      ...
      <div className="w-full flex flex-col items-center justify-center gap-5 px-5 md:px-0 pt-7 sm:pt-14 pb-20 mt-[69px] md:mt-[108px] sm:bg-gray-100">
        {is3ds ? (
          <div
            id="3ds_iframe"
            className="bg-white w-full md:w-[90%] lg:max-w-[1440px] mx-auto rounded-xl min-h-[100px] flex flex-col items-center justify-center pt-28"
          ></div>
        ) : (
      ...
      <div
        className={cn(
          "w-full fixed z-20 bottom-0 left-0 p-2.5 bg-white flex sm:hidden border-t",
